    ...
    public class RecentModel : PageModel
    {
        ...
        private string GetDecryptedCustomerName(Order order)
        {
            ...
            try
            {
                ...
                {
                    return IsValidBase64(order.User.FullName) 
                        ? _encryptionService.Decrypt(order.User.FullName) 
                        : order.User.FullName;
                }
                ...
                {
                    return IsValidBase64(order.User.UserName)
                        ? _encryptionService.Decrypt(order.User.UserName)
                        : order.User.UserName;
                }
                ...
                {
                    return IsValidBase64(order.User.Email)
                        ? _encryptionService.Decrypt(order.User.Email)
                        : order.User.Email;
                }
