    ...
    public class UserManagementModel : PageModel
    {
        ...
        public async Task OnGetAsync(string filter = "all")
        {
            ...
            foreach (var user in users)
            {
                ...
                try 
                {
                    // First check if the string is valid Base64 before attempting to decrypt
                    if (IsValidBase64(user.Email))
                        decryptedEmail = _encryptionService.Decrypt(user.Email);
                        
                    if (IsValidBase64(user.FullName))
                ...
                catch 
                {
                    ...
                    // This handles users created before encryption was implemented
                }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                string encryptedUserName = _encryptionService.Encrypt(sanitizedEmail); // Use email as username
                string encryptedFullName = _encryptionService.Encrypt(sanitizedFullName);
                
                _logger.LogInformation($"Encrypted data: Email={encryptedEmail}, UserName={encryptedUserName}");
